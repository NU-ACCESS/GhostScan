...
class GeometricCalibration:
    def __init__(self, cam, projector, intrinsicCalibFile='CalibrationNumpyData/intrinsic_calibration.npz',
                 imgFile='CalibrationImages/Geometric/geo.PNG',  checker_pattern=(23, 7), checker_size=0.01, checker_pixels=60,
    ...
    def calibrate(self, imgUndistortFile):
        # TODO: run through all of the calibration procedure
        # location of raw distorted image
        #imgFile = self.imgFile
        # location of undistorted image
        #imgUndistortFile = self.imgUndistortFile
        # location of intrinsic matrix npz file
        #intrinsicFile = intrinsicFile
        ...
        #marker_size = 0.04
        board_corner = np.array([np.array([[-self.half_width, -self.half_height, 0], [-self.half_width + 0.04, -self.half_height, 0], [-self.half_width + 0.04, -self.half_height + 0.04, 0], [-self.half_width, -self.half_height + 0.04, 0]]),
                        np.array([[self.half_width - 0.04, -self.half_height, 0], [self.half_width, -self.half_height, 0], [self.half_width, -self.half_height + 0.04, 0], [self.half_width - 0.04, -self.half_height + 0.04, 0]]),
                        np.array([[self.half_width - 0.04, self.half_height - 0.04, 0], [self.half_width, self.half_height - 0.04, 0], [self.half_width, self.half_height, 0], [self.half_width - 0.04, self.half_height, 0]]),
                        np.array([[-self.half_width, self.half_height - 0.04, 0], [-self.half_width + 0.04, self.half_height - 0.04, 0], [-self.half_width + 0.04, self.half_height, 0], [-self.half_width, self.half_height, 0]]),
                        np.array([[-0.02, -self.half_height, 0], [0.02, -self.half_height, 0], [0.02, -self.half_height + 0.04, 0], [-0.02, -self.half_height + 0.04, 0]]),
                        np.array([[-self.half_width, -0.02, 0], [-self.half_width + 0.04, -0.02, 0], [-self.half_width + 0.04, 0.02, 0], [-self.half_width, 0.02, 0]]),
                        np.array([[self.half_width - 0.04, -0.02, 0], [self.half_width, -0.02, 0], [self.half_width, 0.02, 0], [self.half_width - 0.04, 0.02, 0]])], dtype=np.float32)
        ...
        ax.quiver(display_zero[0], display_zero[1], display_zero[2], display_z[0], display_z[1], display_z[2], length= 0.1)
