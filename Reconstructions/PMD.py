...
def PhaseMeasuringDeflectometry(rawImgPath, addImgPath, normalPath, metric_factor, cameraMatrix, disCoeffs, ret, rc2s, tc2s, half_length, half_height, marker_size):
    ...
    PhaseMapHC, PhaseMapVC = generateDisplayPhaseMap()
    ...
    plt.plot(PhaseMap0[800, 750:2100])
    plt.xlabel("pixel")
    ...
    plt.show()
    fig = plt.figure(figsize=(15,10))
    plt.plot(PhaseMap1[400:1400, 1000])
    plt.xlabel("pixel")
    plt.ylabel("phase value")
    plt.rcParams.update({'font.size': 22})
...
def sinePattern(x, y, nu_x, nu_y, Max=255, Min=30):
    if (Max > 255 or Max < 0):
...
def generateDisplayPhaseMap(correctPhaseWrap=3.8):
    displayHeight = 1080
    displayWidth = 1920
    NumPatterns = 8
    pattern = np.zeros((displayHeight, displayWidth, NumPatterns), dtype=np.float32)

    DEFLECTOMETRY_FREQ = 0.9
    patternSize = max(displayHeight, displayWidth)
    x = np.linspace(1, displayWidth, displayWidth)
    y = np.linspace(1, displayHeight, displayHeight)
    nu_x =  DEFLECTOMETRY_FREQ * 2 * np.pi / patternSize
    nu_y = DEFLECTOMETRY_FREQ * 2 * np.pi / patternSize

    pattern[..., 0:4] = sinePattern(x, y, nu_x, 0, Max=255, Min=30)
    PhaseMap = -1 * np.arctan2((pattern[..., 0] - pattern[..., 2]), (pattern[..., 1] - pattern[..., 3]))
    PhaseMapX = np.angle(np.exp(1j * (PhaseMap - correctPhaseWrap)))

    pattern[..., 4:8] = sinePattern(x, y, 0, nu_y, Max=255, Min=30)
    PhaseMap = -1 * np.arctan2((pattern[..., 4] - pattern[..., 6]), (pattern[..., 5] - pattern[..., 7]))
    PhaseMapY = np.angle(np.exp(1j * (PhaseMap - correctPhaseWrap)))

    return PhaseMapX[0,...], PhaseMapY[...,0]